...
//        monyType.setOnClickListener(this);
    ...
    public int getLayoutId() {
        return R.layout.activity_tranfer_accounts;
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.trade_erweima_view:
                //动态权限申请
                if (ContextCompat.checkSelfPermission(TransferAccountsActivity.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(TransferAccountsActivity.this, new String[]{Manifest.permission.CAMERA}, 1);
                } else {
                    goScan();
                }
                break;
            case R.id.trade_contack_people_view:
                ...
                Intent intent = new Intent(TransferAccountsActivity.this, ContactPeopleActivity.class);
...
//                break;
     ...
     */
    private void goScan() {
        Intent intent = new Intent(TransferAccountsActivity.this, CaptureActivity.class);
...
//           createTrade();
    ...
    private void payDialog() {
        ...
        if (new BigDecimal(numVal).compareTo(new BigDecimal(leftVal)) > 0) {
            Toast.makeText(TransferAccountsActivity.this, getString(R.string.trade_money_to_much), Toast.LENGTH_SHORT).show();
            return;
        }

        if (new BigDecimal(numVal).compareTo(BigDecimal.ZERO) == 0) {
            Toast.makeText(TransferAccountsActivity.this, getString(R.string.trade_money_to_low), Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        try {
            if (numVal.contains(".")) {
                ...
                if (afterPoint.length() > 6) {
                    Toast.makeText(TransferAccountsActivity.this, getString(R.string.wallet_trade_after_notice), Toast.LENGTH_SHORT).show();
                    return;
                }
...
//        }
        ...
        if (!TextUtils.isEmpty(beizhuVal) && beizhuVal.length() > 30) {
            Toast.makeText(TransferAccountsActivity.this, getString(R.string.trade_beizhu_notice), Toast.LENGTH_SHORT).show();
            return;
        }

        BottomDialogView.Builder builder = new BottomDialogView.Builder(TransferAccountsActivity.this);

        View dialogView = (View) LayoutInflater.from(TransferAccountsActivity.this).inflate(R.layout.dialog_pwd_input_two, null);
                                ...
                                try {
                                    ...
                                    if (response != null) {
                                        if (response.getData() != null && response.getCode() == 0) {
                                            ...
                                            try {
                                                new FailZZDialogView().showTips(TransferAccountsActivity.this, getString(R.string.dialog_fail_sign));
                                                btnNext.setEnabled(true);
//                                                dismissDialog();

                                                if (!((Activity) TransferAccountsActivity.this).isFinishing() && !((Activity) TransferAccountsActivity.this).isDestroyed()) {
                                                    mDialog.dismiss();
                                                }
...
//                                    Toast.makeText(TransferAccountsActivity.this, getString(R.string.trade_success), Toast.LENGTH_SHORT).show();
                                        ...
                                        try {
                                            new SuccessDialogView().showTips(TransferAccountsActivity.this, getString(R.string.dialog_success_sign));
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                        }
...
//                                        finish();
                                        ...
                                        new FailZZDialogView().showTips(TransferAccountsActivity.this, response.getMsg());
                                    }
                                } catch (Exception e) {
                                    try {
                                        new FailZZDialogView().showTips(TransferAccountsActivity.this, getString(R.string.dialog_fail_sign));
