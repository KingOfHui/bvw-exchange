 ...
 * 转账页面
 */
public class TransferAccountsTwoActivity extends BaseActivity implements View.OnClickListener {
...
//    private TypefaceTextView monyType;
    ...
    private LinearLayout moneyNoticeLayout;
...
//        EventBus.getDefault().register(this);
        ...
        leftMoneyNoticeView = findViewById(R.id.left_mongy_notice_view);
       moneyNoticeLayout = findViewById(R.id.left_mongy_notice_layout);
...
//        monyType = findViewById(R.id.select_money_type);
        ...
        if (moneyType.equalsIgnoreCase("usdt")) {
            ...
            signContentView.setText(getResources().getString(R.string.trade_account_two_header) + "Ethereum ERC20 USDT" + getResources().getString(R.string.trade_account_two_two_header_three));
        } else {
        ...
        if(!moneyType.equalsIgnoreCase("USDT")){
            moneyNoticeLayout.setVisibility(View.GONE);
        }
...
//        monyType.setOnClickListener(this);
    ...
    public int getLayoutId() {
        return R.layout.activity_tranfer_accounts_two;
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.trade_erweima_view:
                //动态权限申请
                if (ContextCompat.checkSelfPermission(TransferAccountsTwoActivity.this, Manifest.permission.CAMERA) != PackageManager.PERMISSION_GRANTED) {
                    ActivityCompat.requestPermissions(TransferAccountsTwoActivity.this, new String[]{Manifest.permission.CAMERA}, 1);
                } else {
                    goScan();
                }
                break;
            case R.id.trade_contack_people_view:
                ...
                Intent intent = new Intent(TransferAccountsTwoActivity.this, ContactPeopleActivity.class);
                intent.putExtra("type", 1);
                startActivity(intent);
                break;
            case R.id.layBack:
                TransferAccountsTwoActivity.this.finish();
                break;
            case R.id.btnNext:
                payDialog();
                break;
...
//                break;
     ...
     */
    private void goScan() {
        Intent intent = new Intent(TransferAccountsTwoActivity.this, CaptureActivity.class);
...
//           createTrade();
    ...
    private void payDialog() {
        ...
        if (new BigDecimal(numVal).compareTo(new BigDecimal(leftVal)) > 0) {
            Toast.makeText(TransferAccountsTwoActivity.this, getString(R.string.trade_money_to_much), Toast.LENGTH_SHORT).show();
            return;
        }

        if (new BigDecimal(numVal).compareTo(BigDecimal.ZERO) == 0) {
            Toast.makeText(TransferAccountsTwoActivity.this, getString(R.string.trade_money_to_low), Toast.LENGTH_SHORT).show();
            return;
        }
        ...
        try {
            if (numVal.contains(".")) {
                ...
                if (afterPoint.length() > 6) {
                    Toast.makeText(TransferAccountsTwoActivity.this, getString(R.string.wallet_trade_after_notice), Toast.LENGTH_SHORT).show();
                    return;
                }
...
//        }
        ...
        if (!TextUtils.isEmpty(beizhuVal) && beizhuVal.length() > 30) {
            Toast.makeText(TransferAccountsTwoActivity.this, getString(R.string.trade_beizhu_notice), Toast.LENGTH_SHORT).show();
            return;
        }

        BottomDialogView.Builder builder = new BottomDialogView.Builder(TransferAccountsTwoActivity.this);

        View dialogView = (View) LayoutInflater.from(TransferAccountsTwoActivity.this).inflate(R.layout.dialog_pwd_input_two, null);
        ...
        sureBtn.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (null == contentVal || contentVal.length() == 0) {
                    Toast.makeText(TransferAccountsTwoActivity.this, getString(R.string.empty_pwd), Toast.LENGTH_SHORT).show();
                    return;
                } else {
                    ...
                    if (contentVal.equals(walletModel.getPassword())) {
                        ...
                        Toast.makeText(TransferAccountsTwoActivity.this, getString(R.string.wrong_pwd), Toast.LENGTH_SHORT).show();
                    }
                }
            }
        });
                                ...
                                try {
                                    ...
                                    if (response != null) {
                                        if (response.getData() != null && response.getCode() == 0) {
                                            ...
                                            try {
                                                new FailZZDialogView().showTips(TransferAccountsTwoActivity.this, response.getMsg());
                                                btnNext.setEnabled(true);
//                                                dismissDialog();

                                                if (!((Activity) TransferAccountsTwoActivity.this).isFinishing() && !((Activity) TransferAccountsTwoActivity.this).isDestroyed()) {
                                                    mDialog.dismiss();
                                                }
...
//                                        Toast.makeText(TransferAccountsActivity.this, getString(R.string.trade_wrong_data), Toast.LENGTH_SHORT).show();
                                        try {
                                            new FailZZDialogView().showTips(TransferAccountsTwoActivity.this, getString(R.string.dialog_fail_sign));
                                            btnNext.setEnabled(true);
//                                            dismissDialog();
                                            if (!((Activity) TransferAccountsTwoActivity.this).isFinishing() && !((Activity) TransferAccountsTwoActivity.this).isDestroyed()) {
                                                mDialog.dismiss();
...
//        String signValss = BitcoinjKit.signMessageBy58(msg, priVal);
        ...
        if (publicAddressVal == null) {
            Toast.makeText(TransferAccountsTwoActivity.this, "获取地址失败，请退出重试", Toast.LENGTH_SHORT).show();
            return;
        }
...
//                                    Toast.makeText(TransferAccountsActivity.this, getString(R.string.trade_success), Toast.LENGTH_SHORT).show();
                                        ...
                                        try {
                                            new SuccessDialogView().showTips(TransferAccountsTwoActivity.this, getString(R.string.dialog_success_sign));
                                        } catch (Exception e) {
                                            e.printStackTrace();
                                        }
...
//
//                                        Toast.makeText(TransferAccountsTwoActivity.this, response.getMsg(), Toast.LENGTH_SHORT).show();
//
//                                        finish();
...
////                EventBus.getDefault().post(new PushEvent());
//
//                Intent tradeIntent = new Intent(TransferAccountsTwoActivity.this, MessageCenterActivity.class);
...
//        showDialog(getString(R.string.load_data));
        ...
        OkGo.<String>get(ConfigNetWork.JAVA_API_URL + UrlPath.MONEY_LEFT_URL)
                .tag(TransferAccountsTwoActivity.this)
                .headers("Chain-Authentication", addressVals + "#" + msg + "#" + signVal)
                .execute(new StringCallback() {
                    @Override
                    public void onSuccess(Response<String> backresponse) {
                        if (backresponse != null) {
                            ...
                            if (backVal != null) {
                                Gson gson = new Gson();
                                try {
                                    DayLeftMoneyResponse response = gson.fromJson(backVal, DayLeftMoneyResponse.class);
                                    if (response != null && response.getCode() == 0) {
                                        if (response.getData() != null) {
                                            setLeftMoneyVal(response.getData());
                                        }
                                    }else {
                                        leftMoneyNoticeView.setVisibility(View.GONE);
                                        moneyNoticeLayout.setVisibility(View.GONE);
                                    }
                                } catch (Exception e) {
                                    e.printStackTrace();
...
//                        dismissDialog();
    ...
    private void setLeftMoneyVal(DayLeftMoneyResponse.DayLeftMoneyModel dayLeftMoneyModel){
        if(!TextUtils.isEmpty(dayLeftMoneyModel.getMsg())){
            leftMoneyNoticeView.setText(dayLeftMoneyModel.getMsg());
        }else {
            leftMoneyNoticeView.setVisibility(View.GONE);
            moneyNoticeLayout.setVisibility(View.GONE);
        }
    ...
    private void setAddress(List<PublicAddressResponse.PublicAddressModel> data) {
        for (int i = 0; i < data.size(); i++) {
            ...
            if (addressModel != null) {
                if (addressModel.getK().equalsIgnoreCase("org_address_withdraw")) {
