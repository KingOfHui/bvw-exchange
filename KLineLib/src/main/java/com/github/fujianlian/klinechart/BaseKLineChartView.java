...
/**
 * k线图 Created by tian on 2016/5/3.
 */
public abstract class BaseKLineChartView extends ScrollAndScaleView {
    ...
    private void drawText(Canvas canvas) {
        ...
        //--------------画上方k线图的值-------------
        if (mMainDraw != null) {
            canvas.drawText(formatValue(mMainMaxValue), mWidth - calculateWidth(formatValue(mMainMaxValue)) - 20, baseLine + mMainRect.top, mTextPaint);
            canvas.drawText(formatValue(mMainMinValue), mWidth - calculateWidth(formatValue(mMainMinValue)) - 20, mMainRect.bottom - textHeight + baseLine, mTextPaint);
            ...
            for (int i = 1; i < mGridRows; i++) {
                ...
                canvas.drawText(text, mWidth - calculateWidth(text) - 20, fixTextY(rowSpace * i + mMainRect.top), mTextPaint);
            }
        }
        //--------------画中间子图的值-------------
        if (mVolDraw != null) {
            canvas.drawText(mVolDraw.getValueFormatter().format(mVolMaxValue),
                    mWidth - calculateWidth(formatValue(mVolMaxValue)) - 20, mMainRect.bottom + baseLine, mTextPaint);
        ...
        if (mChildDraw != null) {
            canvas.drawText(mChildDraw.getValueFormatter().format(mChildMaxValue),
                    mWidth - calculateWidth(formatValue(mChildMaxValue)) - 20, mVolRect.bottom + baseLine, mTextPaint);
...
//        this.newDate = date;
    ...
    private void drawCurrentData(Canvas canvas, float textHeight) {
        ...
        // 画Y值
        IKLine point = (IKLine) getItem(mAdapter.getCount() - 1);
...
//        setNewInfo("  " + mAdapter.getDate(mItemCount - RANG_ITEM - 1) + s);
        setNewPosition(mAdapter.getCount() - 1);

//        //绘制最大值和最小值
//        float x = translateXtoX(getX(getNewPosition()));
//        float y = getMainY(point.getClosePrice());
//        String LowString = "── " + point.getClosePrice();
//
//        //计算显示位置
//        //计算文本宽度
//        int lowStringWidth = calculateMaxMin(LowString).width();
...
//            canvas.drawPath(path, mSelectorFramePaint);
            canvas.drawText(text, x, fixTextY1(y), mMaxMinPaint);
        } else {
